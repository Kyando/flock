  ...
  private openHelp(): void {
    ...
    openModal({
      ...
      body: h(
        'div',
        { class: 'help' },
        h('p', {}, 'Leve cada ', h('b', {}, 'ovelha'), ' até um ', h('b', {}, 'curral'), '. Arraste o dedo sobre uma ovelha ou fardo: ele desliza até bater em algo.'),
        h(
          'ul',
          { class: 'legend' },
          item(legendTile({ kind: 'pen' }, 'sheep'), 'Ovelha e curral.', 'Toda ovelha precisa terminar num curral.'),
          item(
            h('span', { class: 'legend-pair' }, legendTile({ kind: 'grass' }, 'sheep'), legendTile({ kind: 'grass' }, 'sheep')),
            'Pulo.',
            'Arraste uma ovelha na direção de outra colada nela: ela pula por cima, cai duas casas adiante e para. Deslizando, ela nunca pula.',
          ),
          item(legendTile({ kind: 'grass' }, 'hay'), 'Feno.', 'Desliza como as ovelhas e serve de parede. Não dá para pular.'),
          item(legendTile({ kind: 'rock' }), 'Pedra.', 'Bloqueia o caminho.'),
